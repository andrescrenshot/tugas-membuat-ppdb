...
const Siswa = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:5000/menu2");
        setData(res.data);
      } catch (err) {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://localhost:5000/menu2/${id}`);
      ...
      <table className="min-w-full text-sm text-left text-gray-800">
        <thead className="bg-gradient-to-r from-sky-500 to-blue-400 text-white uppercase text-xs">
          <tr>
            ...
            <th className="px-4 py-3">Nomor IS</th>
          ...
          {data.length > 0 ? (
            data.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-3 border-t">{item.tempat_tanggal}</td>
                <td className="px-4 py-3 border-t">{item.nomor_induk}</td>
                <td className="px-4 py-3 border-t">{item.nomor_is}</td>
                <td className="px-4 py-3 border-t">{item.alamat_lengkap}</td>
                <td className="px-4 py-3 border-t">{item.rata_nilai}</td>
                <td className="px-4 py-3 border-t text-center space-x-2">
                  <button
                    onClick={() => navigate(`/Menu2edit/${item.id}`)}
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 bg-gradient-to-r from-sky-400 to-blue-500 text-white p-4 rounded-xl shadow-md">
        <h2 className="text-2xl font-semibold tracking-wide">Daftar Siswa Jalur Zonasi</h2>
        <button
          onClick={() => navigate("/C")}
